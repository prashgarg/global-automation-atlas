...
- one generic ordinary-workplace benchmark context

Scope:
- assess the task in a generic ordinary workplace benchmark around 2026
- use 2026 as the fixed evaluation year for this prompt variant
- no frontier R&D assumptions
- no bespoke systems unless already common in ordinary workplace settings

Generic benchmark rule:
- "Generic ordinary workplace benchmark" means commercially plausible deployment under ordinary conditions that are neither frontier-only nor unusually resource-constrained
- this is not a country benchmark and not an income-group benchmark
- do not import country-specific assumptions about labor costs, infrastructure, regulation, or institutions

Feasibility:
- feasible only if capability exists in commercial products or services, or widely used open-source
- deployment must be plausible in ordinary workplaces under the generic benchmark context
...
A task has economic automation exposure when currently available technology creates a credible labor-reallocation margin for a nontrivial share of the task core in the generic benchmark context: technology can perform that share at sufficient quality and reliability, and at low enough effective cost, that typical employers would reduce human labor input on that share.
...
- if the task core can be credibly reassigned away from labor for a nontrivial share of the task in the generic benchmark context, keep that exposure judgment even when the mechanism is ordinary software rather than frontier AI
- conversely, do not assign high exposure merely because the mechanism uses AI, optimization, or another advanced-seeming technology if cost, reliability, workflow fit, or ordinary deployment conditions do not support real labor reallocation
...
- predominant margin: which route is more likely to characterize typical deployment for this task in the generic benchmark context around 2026
...
- `margin` should describe the predominant likely route in typical deployment in the generic benchmark context around 2026
...
- it should explain the main economic reason for the assigned exposure and route in the generic benchmark context, while remaining consistent with the dominant channel
- when relevant, mention the core economic reason and the main generic deployment reason; if exposure_level <= 1, explain why the task remains assistive or non-automated rather than naming a predominant route
...
- if two channels seem possible, choose the one that best describes what employers are economically buying when they automate this task in ordinary settings
